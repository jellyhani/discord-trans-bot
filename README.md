...
- **스마트 하이브리드 모델 (Mentor Engine)**: 
  - 기본: `gpt-4.1-mini` (초고속, 저비용)
  - 문맥 필요 시: `gpt-5-mini` (추론 기반 정밀 번역)
- **페르소나 시각화 (Persona Visualizer)**: 유저의 대화 데이터를 분석하여 '본인의 자아'를 시각화하고 심층 분석 보고서를 제공합니다.
...
## 📁 프로젝트 구조
- `bot.py`: 메인 실행 파일.
- `core/`: MentorEngine, 번역 엔진, 오타 감지 등 핵심 AI 및 처리 로직.
- `database/`: SQLite DB 및 세션, 페르소나, 사용자 설정 관리.
- `utils/`: 로그 매니저, 사용량 추적, API(날씨/금융) 유틸리티.
- `cogs/`: 기능별 모듈 (persona.py, mentor.py, routines.py 등).
...
- `/draw-me`: **[페르소나]** 본인의 대화 데이터를 분석하여 시각화된 결과와 분석 보고서 수신.
